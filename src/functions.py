...
class GaussianKernel():
    ...
    def simulate_sigma(self, sigma):
        if self.sigma_kernel=="TruncNorm":
            return([truncnorm.rvs(-sigma[0]/self.std_sigma, sigma[0]+10*self.std_sigma, loc=sigma[0], scale=self.std_sigma), \
    ...
    def evaluate(self, w, mu , sigma, w_ctr, mu_ctr, sigma_ctr, log=False):
        if log:
            return(self.evaluate_w(w, w_ctr,True)+self.evaluate_mu(mu, mu_ctr,True)+self.evaluate_sigma(sigma,sigma_ctr, True))
        else:
            return(self.evaluate_w(w, w_ctr,False)*self.evaluate_mu(mu, mu_ctr,False)*self.evaluate_sigma(sigma,sigma_ctr, False))
    ...
    def evaluate_sigma(self, sigma, sigma_ctr, log=False):
        if self.sigma_kernel=="TruncNorm":
            if not(log):
                return(truncnorm.pdf(x=sigma[0], a=-sigma_ctr[0]/self.std_sigma,b= sigma_ctr[0]+10*self.std_sigma, loc=sigma_ctr[0], scale=self.std_sigma)+ \
                ...
                return(truncnorm.logpdf(x=sigma[0], a=-sigma_ctr[0]/self.std_sigma,b= sigma_ctr[0]+10*self.std_sigma, loc=sigma_ctr[0], scale=self.std_sigma)* \
                truncnorm.logpdf(x=sigma[1],a=-sigma_ctr[1]/self.std_sigma, b=sigma_ctr[1]+10*self.std_sigma, loc=sigma_ctr[1], scale=self.std_sigma))                 
        elif self.sigma_kernel=="LogNorm":
            if not(log):
